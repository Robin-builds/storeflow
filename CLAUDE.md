...
**Última actualización:** Julio 2026 — Feature "Trazabilidad de caducidad" (lotes) en progreso: Sesiones 1–4/5 completas y validadas en dispositivo físico (esquema, Supabase+sync, toggle "Es perecedero" en Productos, entrada con lote + salida FEFO en Movimientos). Solo falta Sesión 5 (Alertas "Próximos a vencer"). Card de búsqueda de productos en Dashboard también completa y validada, sin mergear todavía.
...
**Rama activa:** `feat/lotes-ui-movimientos` (rama apilada: `feat/lotes-esquema` → `feat/lotes-supabase-sync` → `feat/lotes-ui-producto` → `feat/lotes-ui-movimientos`, sale de `main`) — Sesión 4/5 de la feature de trazabilidad de caducidad. `main` tiene el sistema de temas composable, la paginación "Cargar más" y el fix de fallback de downgrade de Room ya mergeados/commiteados.
...
- `feat/lotes-esquema` (`9115e31`) → `feat/lotes-supabase-sync` (`0ec36fa`) → `feat/lotes-ui-producto` (`896abc7`) → `feat/lotes-ui-movimientos` (`e4a3552`) — trazabilidad de caducidad (lotes). Sesiones 1–4 de 5 completas y validadas en dispositivo físico + Supabase real. Solo falta Sesión 5 (Alertas "Próximos a vencer") antes de mergear a `main`.
...
- **LoteRepository** — `observarPorProducto(productoId)`, `obtenerConStockFefo(productoId)`, `crear(productoId, empresaId, fechaCaducidad, numeroLote?)` (retorna el `LoteEntity` creado — `MovimientoRepository.registrarEntrada()` usa ese `id` como `lote_id` del movimiento). Consumido desde `MovimientosScreen` (Sesión 4).
...
- 🥫 **Trazabilidad de caducidad / Lotes** (ramas `feat/lotes-esquema` → `feat/lotes-supabase-sync` → `feat/lotes-ui-producto` → `feat/lotes-ui-movimientos`, sin mergear, **Sesiones 1–4/5 completas**) — `ProductoEntity.es_perecedero` (toggle en el form de producto) + `LoteEntity`/`LoteDao`/`LoteRepository` + `MovimientoEntity.lote_id` (nullable, SET_NULL). Stock residual por lote calculado igual que el stock global, vía `LoteDao.obtenerConStockFefo` (orden `fecha_caducidad ASC`). Sync completo (push genérico + pull) contra la tabla `lotes` en Supabase, validado con datos reales. `MovimientosScreen` pide fecha de caducidad (`DatePicker`) + número de lote solo en Entrada de productos perecibles; Salida aplica FEFO automático, descontando primero del lote que vence antes y cruzando a lotes siguientes si la cantidad lo supera (validado con split exacto 6+4=10 en dispositivo físico). **Falta solo la Sesión 5** — sección "Próximos a vencer" en Alertas.
...
LOTES / TRAZABILIDAD CADUCIDAD S1-S4: ✅ Esquema (S1), Supabase+sync (S2), toggle perecedero en Productos (S3), entrada con lote + salida FEFO en Movimientos (S4) — todas validadas en dispositivo físico + Supabase real, rama feat/lotes-ui-movimientos sin mergear
LOTES / TRAZABILIDAD CADUCIDAD S5:     ☐ Pendiente — sección "Próximos a vencer" en Alertas
```
...
### Trazabilidad de caducidad / Lotes (🔶 en progreso, 4/5 sesiones)
...
| Lotes S3 — toggle "Es perecedero" (rama `feat/lotes-ui-producto`) | Moto G60 ✅, confirmado en Supabase (`es_perecedero=true` + atributo `formato_medida` juntos) | "No deja guardar" en un producto puntual → root cause: atributo personalizado obligatorio sin valor (validación preexistente del formulario, no relacionada al toggle) |
| Lotes S4 — entrada con lote + salida FEFO (rama `feat/lotes-ui-movimientos`) | Moto G60 ✅, split FEFO confirmado vía logcat + Supabase (6+4=10, lote más próximo a vencer agotado primero) | Falsa alarma en la primera ronda de prueba: "no toma el lote correcto" en un producto con mucho historial de pruebas manuales previas — investigado con logging temporal, resultó ser que ambos lotes ya estaban agotados localmente, no un bug de la query FEFO |
