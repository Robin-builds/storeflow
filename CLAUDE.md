...
**Última actualización:** Julio 2026 — Feature "Trazabilidad de caducidad" (lotes) en progreso: Sesión 1 (esquema Room v7→v8) y Sesión 2 (Supabase + sync) completas y validadas en dispositivo físico; Sesiones 3–5 (UI) pendientes. Card de búsqueda de productos en Dashboard también completa y validada, sin mergear todavía.
...
**Rama activa:** `feat/lotes-supabase-sync` (rama sobre `feat/lotes-esquema`, que a su vez sale de `main`) — Sesión 2/N de la feature de trazabilidad de caducidad. `main` tiene el sistema de temas composable, la paginación "Cargar más" y el fix de fallback de downgrade de Room ya mergeados/commiteados.
**Ramas sin mergear a `main`:**
- `feat/dashboard-buscar-producto` (`3a277ec`) — card de búsqueda de productos en el Dashboard (todas las bodegas, solo lectura). Completa y validada en dispositivo físico, pendiente de mergear.
- `feat/lotes-esquema` (`9115e31`) → `feat/lotes-supabase-sync` (`0ec36fa`) — trazabilidad de caducidad (lotes). Sesiones 1 y 2 de 5 completas y validadas en dispositivo físico + Supabase real. Sesiones 3 (UI producto), 4 (UI movimientos + FEFO) y 5 (alertas) pendientes, cada una en su propia rama antes de mergear a `main`.
...
- **Trazabilidad de caducidad (lotes, en progreso, Sesiones 1–2/5 completas)** — `ProductoEntity.es_perecedero` (default `false`) marca qué productos llevan lotes; `LoteEntity` (`producto_id` + `empresa_id` FK CASCADE, `numero_lote` opcional, `fecha_caducidad`) vive a la par de `MovimientoEntity`, que gana `lote_id` nullable (FK → `lotes`, `ON DELETE SET NULL` — la inmutabilidad de movimientos no cambia; `null` = no perecedero o entrada/ajuste sin lote). Sin Órdenes de Compra en la app, el lote se crea al registrar una **Entrada** (no en una recepción de OC, que no existe acá). Salida usa **FEFO automático** (`LoteDao.obtenerConStockFefo`, orden por `fecha_caducidad ASC`) — puede generar más de un `MovimientoEntity` por acción de Salida si la cantidad cruza más de un lote. **Nota de migración:** `ALTER TABLE ... ADD COLUMN` en SQLite no registra una FK a menos que se declare `REFERENCES` inline en el mismo `ALTER` — un primer intento sin eso crasheaba con `Migration didn't properly handle: movimientos` (ver `MIGRATION_7_8`)
...
- **ProductoRepository** — `obtenerContexto()`, `observarProductos/BajoMinimo/SinMovimientoReciente/Producto`, `observarProductosDeEmpresa(empresaId)` (todas las bodegas, solo lectura — búsqueda global del Dashboard), `obtenerPorId`, `crear()` (valida SKU único, crea movimiento inicial si aplica, guarda atributos), `actualizar()`, `eliminar()`, `eliminarVarios()`, `transferirSeleccionados()`.
- **MovimientoRepository** — `observarProducto`, `observarMovimientos`, `registrarEntrada/Salida/Ajuste` (todas validan `nota` no vacía y reglas de cantidad/stock). Pendiente (Sesión 4 de lotes): entrada con `fecha_caducidad`/creación de lote si `producto.es_perecedero`, salida FEFO multi-lote.
...
- **LoteRepository** — `observarPorProducto(productoId)`, `obtenerConStockFefo(productoId)`, `crear(productoId, empresaId, fechaCaducidad, numeroLote?)` (retorna el `LoteEntity` creado — quien registre la Entrada usa ese `id` como `lote_id` del movimiento). Sin UI todavía (Sesiones 3–4 pendientes).
...
| **DashboardViewModel** | AuthSessionDao, ProductoRepository | `nombreUsuario`, `rolActual: StateFlow<Rol>`, `sinMovimientoReciente: StateFlow<List<Producto>>`, `busquedaGlobal`/`resultadosBusquedaGlobal: StateFlow<List<ProductoConStockYBodega>>` (búsqueda de productos en todas las bodegas, máx. 20 resultados — rama `feat/dashboard-buscar-producto`, sin mergear) | — |
...
- 🔍 **Búsqueda de productos en Dashboard** (rama `feat/dashboard-buscar-producto`, sin mergear) — card inline expandible en `DashboardScreen`, busca por nombre/SKU en **todas las bodegas** de la empresa (`ProductoDao.observarConStockPorEmpresa`), muestra nombre/precio/stock/bodega, solo lectura (sin editar ni mover stock — eso sigue siendo exclusivo de Productos). Visible para ambos roles. Botón "Ver en Productos" navega con la búsqueda precargada vía argumento de ruta opcional (`Rutas.productosConBusqueda()`), sin cambiar la bodega activa. Validada en dispositivo físico.
- 🥫 **Trazabilidad de caducidad / Lotes** (ramas `feat/lotes-esquema` → `feat/lotes-supabase-sync`, sin mergear, **Sesiones 1–2/5 completas**) — `ProductoEntity.es_perecedero` + `LoteEntity`/`LoteDao`/`LoteRepository` + `MovimientoEntity.lote_id` (nullable, SET_NULL). Stock residual por lote calculado igual que el stock global, vía `LoteDao.obtenerConStockFefo` (orden `fecha_caducidad ASC`). Sync completo (push genérico + pull) contra la tabla `lotes` en Supabase, validado con datos reales. **Sin UI todavía** — faltan: toggle "Es perecedero" en el form de producto (Sesión 3), fecha de caducidad en el form de Entrada + lógica de Salida FEFO multi-lote en Movimientos (Sesión 4), sección "Próximos a vencer" en Alertas (Sesión 5).
...
BÚSQUEDA PRODUCTOS EN DASHBOARD:      ✅ Completo — validada en dispositivo físico, rama feat/dashboard-buscar-producto sin mergear (ver Features post-MVP)
LOTES / TRAZABILIDAD CADUCIDAD S1-S2: ✅ Sesiones 1 (esquema Room v7→v8) y 2 (Supabase + sync) completas, validadas en dispositivo físico + Supabase real — rama feat/lotes-supabase-sync sin mergear
LOTES / TRAZABILIDAD CADUCIDAD S3-S5: ☐ Pendiente — UI producto (S3), UI movimientos + FEFO (S4), alertas (S5)
```
...
### Trazabilidad de caducidad / Lotes (🔶 en progreso, 2/5 sesiones)
...
- **S3** (pendiente) — UI producto: toggle "Es perecedero" en el formulario de `ProductosListScreen`.
- **S4** (pendiente) — UI movimientos: formulario de Entrada pide `fecha_caducidad`/`numero_lote` si `es_perecedero`; Salida usa FEFO automático (puede generar varios `MovimientoEntity`, uno por lote afectado).
- **S5** (pendiente) — Alertas: sección "Próximos a vencer" junto a la de stock mínimo existente.
...
| Búsqueda de productos en Dashboard (rama `feat/dashboard-buscar-producto`) | Moto G60 ✅ | ninguno |
| Lotes S1 — esquema Room v7→v8 (rama `feat/lotes-esquema`) | Moto G60 ✅, forzando upgrade real v7→v8 (no solo instalación limpia) | 1er intento de `MIGRATION_7_8` crasheaba (`Migration didn't properly handle: movimientos`) — `ALTER TABLE ADD COLUMN lote_id TEXT` sin `REFERENCES` no registra la FK que Room espera; corregido con `REFERENCES lotes(id) ON DELETE SET NULL` inline |
| Lotes S2 — Supabase + sync (rama `feat/lotes-supabase-sync`) | Moto G60 ✅, sync real contra Supabase (proyecto "StockFlow", ~10k productos/movimientos) | ninguno — producto sincronizado con `es_perecedero=false` explícito, movimiento con `lote_id=null` (se omite correctamente), sin romper sync de productos/movimientos existente |
