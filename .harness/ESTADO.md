...
> **Última actualización:** 23/08/2026 — `usuario_id` en movimientos (migración Room 8→9), validado en dispositivo físico y mergeado a `main`. Ver punto 12. Sesión larga, con foco mayormente en `stockflow-web` (ver `.harness/ESTADO.md` de ese repo para el detalle completo).
>
> 12. **`usuario_id` en `MovimientoEntity`** — cierra el gap documentado en `PUENTE.md`: la web (columna "Usuario" en Movimientos, agregada el 23/08) mostraba "—" para todo porque Android nunca escribía este campo. Migración Room `MIGRATION_8_9` (mismo patrón que `lote_id` en la 7→8): `ALTER TABLE movimientos ADD COLUMN usuario_id TEXT REFERENCES usuarios(id) ON DELETE SET NULL`. `MovimientoRepository` ahora inyecta `AuthSessionDao` y setea `usuario_id` en las 4 rutas de creación de movimientos + el stock inicial de `ProductoRepository.crear()`. Propagado a Supabase vía `SyncPayloads`, leído de vuelta vía `PullDtos`. Test nuevo `MovimientoRepositoryTest.kt` (no existía antes, 4 tests). **Validado en dispositivo físico (Moto G60):** upgrade real v8→v9 (no reinstalación limpia) sin crash, movimiento nuevo sincronizado, columna Usuario en la web mostrando el nombre real. Único bache real del proceso: la primera prueba falló porque el `refresh_token` de la sesión del dispositivo estaba vencido (HTTP 400 al refrescar) — sin relación con el cambio, se resolvió cerrando/reabriendo sesión. Commit mergeado a `main` y pusheado a `origin/main`. Plan completo en `docs/plans/2026-08-23-usuario-id-movimientos.md`.
>
> **Nota — plan y ejecución del lado web (mismo día 23/08):** en la misma sesión se hizo bastante trabajo en `stockflow-web` (5 páginas nuevas de solo lectura, fix crítico de env vars de Supabase en Vercel — apuntaban al proyecto viejo desde la migración del 08/08 sin que nadie lo notara, paginación real server-side, sistema de temas, logo real, etc.) — el detalle completo vive en `stockflow-web\.harness\ESTADO.md`, no se duplica acá. Lo único con impacto directo en este repo es el punto 12 de arriba.
>
> 11. **Keystore nuevo encontrado + AAB firmado subido a Play Console** — el keystore que el usuario había creado el 15/08 no estaba donde `keystore.properties` apuntaba (`Desktop\storeflow-release.jks`, que no existe); apareció como `mi_keystore.jks` en 3 copias (`Desktop`, `Documents\StoreFlow - importante\jks`, `Documents\Obsidian Vault` — esta última es el keystore VIEJO del 18/07, no usar). Credenciales del usuario (alias `storeflow-key`) verificadas con `keytool -list` contra el archivo del 15/08 antes de firmar — coinciden. `keystore.properties` actualizado para apuntar a `Documents\StoreFlow - importante\jks\mi_keystore.jks`. Toda esta info quedó consolidada en `Documents\StoreFlow - importante\keystore-info.md` (los dos archivos viejos de esa carpeta, un `.md` y un `.pdf` con la misma info, se borraron a pedido del usuario). Se generó el AAB (`bundleRelease`) y se subió a Play Console → StoreFlow → Prueba interna: **al intentar subir se detectó que ya había una versión `1 (1.0.0)` publicada ahí desde el 15/08** (sin los fixes de sesiones anteriores) — Play Console no permite reusar `versionCode`, así que se subió `versionCode` a **2** / `versionName` a **"1.0.1"** en `app/build.gradle.kts` antes de regenerar el AAB. Publicado como versión **2 (1.0.1)**, 16/08 20:11, "Disponible para testers internos" — **pero el canal no tiene testers asignados todavía** (advertencia de Play Console al publicar), así que por ahora nadie puede instalarla hasta que se configure la pestaña "Testers" del canal de Prueba interna. Google Play App Signing está activo (las versiones se re-firman con la clave de Google al distribuirse; el keystore propio es solo la "upload key").
...
> 8. **Cambio de `applicationId`** (`cl.storeflow.warehouse` → `cl.storeflow.app`) — el listing anterior en Play Store quedó con el keystore perdido; se abandona y se crea uno nuevo desde cero. `namespace` de Kotlin **se mantiene** en `cl.storeflow.warehouse` (solo cambia el identificador de publicación, no el código). `versionCode`/`versionName` en `app/build.gradle.kts` arrancaron en 1/"1.0.0", ahora en **2/"1.0.1"** (ver punto 11). Sin impacto en Supabase (no hay OAuth ni deep links atados al `applicationId`) ni en `AndroidManifest.xml` (usa `${applicationId}` dinámico). Quedan como referencias cosméticas sin actualizar (no bloquean build/publicación): `.harness/CLAUDE.md` (línea de nombre del proyecto), `docs/plans/2026-08-08-reset-password.md`, `.claude/settings.local.json` (rutas de test hardcodeadas). **Ya en `main`** (mergeado el 16/08 junto con los puntos 9-10). **Resuelto (16/08):** keystore nuevo localizado y AAB firmado generado y subido — ver punto 11. `proguard-rules.pro` revisado y está vacío pero `isMinifyEnabled = false` en release, así que no aplica para este build.
>
...
Sin rama activa — `feat/cambio-applicationId-play-store` y `fix/mensaje-error-sin-conexion` ya están mergeadas a `main` (16/08). Ambas siguen existiendo como ramas locales/remotas sin borrar; se pueden eliminar cuando se confirme que no hace falta volver a ellas.

**Estado de publicación:** `applicationId` nuevo (`cl.storeflow.app`) en `main`, AAB firmado (versión 2/1.0.1) subido a Play Console → Prueba interna. **Pendiente:** configurar testers en esa pestaña para que la versión sea instalable — ver punto 11 arriba.
